...
- **While held** — every loaded ByteChest within 32 blocks is highlighted with a tint-colored
  wireframe and a HUD label (chest's custom name, or "ByteChest").
...
- **HUD** — `GlassesHudOverlay` shows widgets when worn; falls back to a "no signal" placeholder
  ...
  channel.
- **Lua demo/test script** — bundled `power-monitor.lua` (`src/main/resources/assets/byteblock/programs/power-monitor.lua`) demonstrates the Smart Glasses widget flow (`addTitle`, `addBar`, `addText`, `set`, `flush`). Run with `apt install power-monitor` then `power-monitor [channel]`.

## Crafting Recipes

The **Computer** is the gating tech and recipe-ingredient for nearly every other ByteBlock device.
Pattern legend: `I`=Iron Ingot, `R`=Redstone, `C`=Computer, `G`=Glass, `T`=Redstone Torch,
`D`=Repeater, `E`=Ender Pearl/Eye, `P`=Paper, `B`=Stone Button, `S`=Stick / String, `F`=Feather,
`W`=Wheat, `Cu`=Copper Ingot.

| Item | Pattern | Key ingredients |
|------|---------|-----------------|
| `computer` | `IGI / RDR / III` | iron + glass + redstone + repeater |
| `peripheral` | `IRI / RCR / ITI` | iron + redstone + redstone torch + computer |
| `gps` | `IEI / RCR / III` | iron + ender pearl + redstone + computer |
| `scanner` | `GEG / RCR / III` | iron + glass + ender eye + redstone + computer |
| `drive` | `III / RCR / III` | iron + redstone + chest |
| `printer` | `IPI / PCP / IRI` | iron + paper + redstone + computer |
| `monitor` | `IGI / GGG / IRI` | iron + glass + redstone |
| `redstone_relay` | `ITI / RDR / ITI` | iron + redstone torch + redstone + repeater |
| `button_panel` | `BBB / IRI / III` | stone buttons + iron + redstone |
| `charging_station` | `CuCuCu / CuRCu / III` | copper + redstone block + iron |
| `byte_chest` | `III / ICI / IRI` | iron + computer + redstone |
| `disk` | shapeless | iron nugget + redstone + paper |
| `glasses` | `GIG / S S / ` | glass pane + iron + string |
| `gps_tool` | ` G  /  R  /  S ` | gps block + redstone + stick |
| `robot_spawn_egg` | `III / ICI / IPI` | iron + computer + piston |
